...
class _FavoriteButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<Set<String>>(
      valueListenable: FavoritesController.instans,
      builder: (context,favoritesId, _) {
        final isFavorite = favoritesId.contains(flowerId);
        return CircleIconButton(
          icon: isFavorite ? Icons.favorite_rounded : Icons.favorite_rounded,
          iconColor: isFavorite ? AppTheme.primary : Colors.black,
          onTap: () => FavoritesController.instans.toggle(flowerId),
        );
      },
    );
  }
